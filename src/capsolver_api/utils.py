...
class AsyncTurnstileSolver:
    ...
    async def _setup_page(
        self, browser, url: str, sitekey: str, action: str = "", cdata: str = ""
    ):
        ...
        await page.route(
            url_with_slash, lambda route: route.fulfill(body=page_data, status=200)
        )
        await page.goto(url_with_slash)

        return page

    async def _get_turnstile_response(
        self, page, max_attempts: int = 10
    ) -> Optional[str]:
    ...
    async def solve(
        self, url: str, sitekey: str, action: str = "", cdata: str = ""
    ):
        """
...
async def get_turnstile_token(
    ...
    # browser_types = ["chromium", "chrome", "camoufox", "msedge"]
    ...
    #     logger.error(
    #         f"You must specify a {COLORS.get('YELLOW')}User-Agent{COLORS.get('RESET')} for Turnstile Solver or use {COLORS.get('GREEN')}camoufox{COLORS.get('RESET')} without useragent"
    #     )
    # else:
